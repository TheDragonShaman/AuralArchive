"""
Module Name: torznab_base_indexer.py
Author: TheDragonShaman
Created: Feb 18 2026
Description:
    Protocol base for all Torznab indexers (Jackett, Prowlarr torrent endpoint).
    Owns all Torznab XML parsing, capabilities parsing, magnet building,
    result normalisation, and shared HTTP helpers.

    Subclasses are responsible only for computing the API endpoint URL and
    passing it to this class via the constructor.

Location:
    /services/indexers/torznab_base_indexer.py
...
_LOGGER = get_module_logger("Service.Indexers.Torznab")
...
@dataclass
class TorznabItem:
    """Normalised representation of a single Torznab feed item."""
...
class TorznabBaseIndexer(BaseIndexer):
    """
    Protocol base for Torznab indexers.

    Implements the full Torznab search/caps/result pipeline.
    Subclasses must compute the Torznab API endpoint URL and pass it as
    ...
    Example subclass::
        ...
        class JackettIndexer(TorznabBaseIndexer):
            def __init__(self, config, *, logger=None):
                config = dict(config)
                api_endpoint = f"{config['base_url']}/api/v2.0/indexers/all/results/torznab"
    ...
    def _probe_search_endpoint(self) -> Optional[str]:
        ...
        try:
            ...
            root = ET.fromstring(response.content)
            # A valid Torznab response must contain an RSS channel element.
            channel = root.find("channel")
            if channel is None:
                return "Search probe returned XML with no <channel> element"
            self.logger.debug(
                "Torznab search probe succeeded",
                extra={"indexer_name": self.name},
            )
            return None
        except requests.exceptions.Timeout:
    ...
    def search(
        ...
        if not self.is_available():
            self.logger.warning(
                "Indexer unavailable, skipping search",
                extra={"indexer_name": self.name},
            )
            return []
        ...
        try:
            ...
            self.logger.exception(
                "Error searching Torznab indexer",
                extra={"indexer_name": self.name},
            )
        return []
    ...
    def _request(self, params: Dict[str, Any]) -> requests.Response:
        ...
        if response.status_code == 404:
            raise FileNotFoundError(
                f"Torznab endpoint not found: {self.api_endpoint}"
            )
    ...
    def _parse_items(self, root: ET.Element) -> List[TorznabItem]:
        ...
        for element in root.findall(".//item"):
            try:
                ...
                self.logger.debug(
                    "Skipping malformed Torznab item: %s", exc, exc_info=True
                )
        return items

    def _parse_single_item(self, element: ET.Element) -> Optional[TorznabItem]:
        ...
        if not download_url:
            ...
            if info_hash:
                download_url = self._build_magnet_from_attributes(
                    info_hash, attrs, title
                )
        if not download_url:
            self.logger.debug(
                "Skipping item — no .torrent URL or infohash",
                extra={"title": title},
            )
            return None
    ...
    def _select_torrent_url(self, element: ET.Element) -> Optional[str]:
        ...
        return fallback if self._is_torrent_url(fallback, "") else None
    ...
    def _is_torrent_url(self, url: str, content_type: str) -> bool:
        ...
        if "bittorrent" in content_type:
            return True
        parsed = urlparse(url)
        if (parsed.path or "").lower().endswith(".torrent"):
            return True
        query_params = parse_qs(parsed.query)
        filename_params = query_params.get("file") or query_params.get("filename")
        return bool(
            filename_params
            and any(n.lower().endswith(".torrent") for n in filename_params)
        )
    ...
    def _build_magnet_from_attributes(
        ...
        trackers: List[str] = []
        raw = attrs.get("tracker") or attrs.get("trackers")
        if raw:
            tokens = [raw]
            for sep in ["|", ",", ";"]:
                tokens = [sub for tok in tokens for sub in tok.split(sep)]
        ...
        display_name = attrs.get("dn") or attrs.get("title") or title
        parts = [f"magnet:?xt=urn:btih:{info_hash.lower()}"]
        parts.extend(f"tr={quote(tr, safe=':/?&=')}" for tr in trackers)
        if display_name:
    ...
    def _build_result(self, item: TorznabItem) -> Dict[str, Any]:
        result: Dict[str, Any] = {
            "indexer": self.name,
            "title": item.title,
        ...
        author = media_primary.pop("author", None) or self._extract_author(item.title)
        if author:
            result["author"] = author
        for field_name in ("narrator", "language", "series", "sequence"):
            if media_primary.get(field_name):
                result[field_name] = media_primary.pop(field_name)
        if media_extras:
    ...
    def _extract_media_details(
        ...
        sequence = grab(
            "booknumber", "booknum", "seriesnumber", "volume", "issue", "sequence"
        )
        if sequence:
