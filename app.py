...
def _get_listen_port(default: int = 8765) -> int:
    """Resolve listen port from env with a safe fallback."""
    env_port = os.environ.get("AURALARCHIVE_PORT") or os.environ.get("PORT")
    if env_port:
        try:
            return int(env_port)
        except (TypeError, ValueError):
            logger.warning(
                "Invalid port provided; falling back to default",
                extra={"port_value": env_port, "default_port": default},
            )
    return default
